...
def create_chain_csp(n):
    ...
    #add binary functional:
    for var1 in range(0, len(variables)-1):
        csp.add_binary_potentail(variables[var1], variables[var1+1], variables[var1]^variables[var1+1])
    # Problem 0c
...
class BacktrackingSearch():
    ...
    def get_unassigned_variable(self, assignment):
        ...
        if not self.mcv:
            ...
            # BEGIN_YOUR_CODE (around 10 lines of code expected)
            # unassigned = [v for v in csp.varNames if v not in assignment] 
...
class SchedulingCSPConstructor():
    ...
    def add_quarter_constraints(self, csp):
        ...
        #    for quarter in self.profile.quarters:
        #        csp.add_unary_potential((req, quarter), \
        #            lambda cid: cid not in req.cids or(cid in req.cids and quarter in req.quarters)
        raise Exception("Not implemented yet")
        # END_YOUR_CODE
    def add_request_weights(self, csp):
        ...
        for req in self.profile.requests:
            for quarter in self.profile.quarters:
                csp_var = csp.varNames.index((req, quarter))
                csp.unaryPotentials[csp_var][i] = req.weight for i in range(len(csp.unaryPotentials[csp_var])) if csp.unaryPotentials[csp_var][i]
        # END_YOUR_CODE
    ...
    def add_unit_constraints(self, csp):
        ...
        for cid in courseList:
            for quarter in self.profile.quarters:
                csp.add_variable((cid, quarter), [i for i in
                                 range(self.bulletin.courses[cid].minUnits,
                                 self.bulletin.courses[cid].maxUnits
                                 + 1) + [0]])

        def getPotential(cid):

            def potential(x, y):
                if x == cid:
                    return y >= self.bulletin.courses[x].minUnits and y \
                        <= self.bulletin.courses[x].maxUnits
                return y == 0
        ...
        for req in self.profile.requests:
            for quarter in self.profile.quarters:
                for cid in req.cids:
                    csp.add_binary_potential((req, quarter), (cid,
                            quarter), getPotential(cid))

        maxSum = self.profile.maxUnits
        for quarter in self.profile.quarters:
            varList = []
            for cid in courseList:
                varList.append((cid, quarter))
            sumVar = get_sum_variable(csp, quarter, varList, maxSum)
            csp.add_unary_potential(sumVar, lambda x: x \
                                    >= self.profile.minUnits and x \
